...
```markdown
[![Codemagic build status](https://api.codemagic.io/apps/<app-id>/<workflow-id>/status_badge.svg)](https://codemagic.io/app/<app-id>/builds)
```
...
```markdown
[![Codemagic build status](https://api.codemagic.io/apps/6a37138f33b051c12a7b3e50/release-workflow/status_badge.svg)](https://codemagic.io/app/6a37138f33b051c12a7b3e50/builds)
```
...
**Q: Bagaimana cara membuat detail *build* aplikasi saya menjadi Public (bisa dilihat semua orang ketika mengklik *badge* tanpa harus login)?**

**A:** Secara *default*, aplikasi di Codemagic bersifat **Private**. Berdasarkan pembaruan antarmuka (UI) Codemagic terbaru, opsi *"Make application public"* telah **dihilangkan** dari *Repository Settings* untuk pengguna akun *Personal/Free Tier* (kemungkinan fitur ini dialihkan khusus untuk pengguna *Teams*/berbayar). 

Namun **jangan khawatir!** Gambar visual dari *badge* Anda (ikon hijau kesuksesan atau merah kegagalan) yang diambil dari API `status_badge.svg` **tetap bersifat publik** dan akan selalu tampil sempurna di halaman depan GitHub Anda untuk dilihat semua orang.

Satu-satunya efek pembatasan *Private* ini adalah: jika orang lain/pengunjung (yang tidak login ke akun Anda) mengklik *badge* tersebut, mereka tidak akan bisa masuk ke dalam dasbor Codemagic Anda untuk mengintip *log* server (mereka hanya akan disuguhkan halaman *Not Found* atau *null*). Hal ini pada dasarnya **jauh lebih baik dan sangat disarankan** demi menjaga keamanan dan privasi rahasia konfigurasi CI/CD proyek Anda!
